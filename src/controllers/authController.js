...
// Step 2: Verify OTP and create user if new
export const verifyOTPAndLogin = async (req, res) => {
  try {
    const { mobile, code } = req.body;
    if (!mobile || !code) return res.status(400).json({ error: 'Mobile and OTP code required' });
    ...
    // Generate JWT token (without PIN yet)
    const token = jwt.sign({ userId: user.id, mobile: user.mobile }, JWT_SECRET, { expiresIn: '7d' });

    res.json({ message: 'OTP verified', token, userId: user.id, isPinSet: !!user.pinHash });
  } catch (error) {
